...
const uploadPrescriptionFile = async (req, res) => {
  try {
    ...
    const doctorId =
      req.user?.id || req.query.doctorId || req.headers['x-doctor-id'];
    ...
    await Consultation.update(
      { prescription: uploadResult.fileUrl },
...
const createCustomPrescription = async (req, res) => {
  try {
    ...
    let patientId =
      req.query.userId || req.headers['x-user-id'] || bodyPatientId;
    ...
    await Consultation.update(
      { prescription: pdfResult.fileUrl },
      { where: { id: consultationId } }
    );
...
const getPatientPrescriptions = async (req, res) => {
  try {
    ...
    const patientId =
      req.user?.id || req.query.userId || req.headers['x-user-id'];
  ...
  try {
    ...
    if (!prescription) {
      return res.status(404).json({
        success: false,
        message:
          'Prescription not found or you do not have permission to delete it',
      });
    }
